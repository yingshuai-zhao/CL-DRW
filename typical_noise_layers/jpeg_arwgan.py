...
def rgb_to_ycbcr_jpeg(image: torch.Tensor) -> torch.Tensor:
...
# -1. YCbCr -> RGB
def ycbcr_to_rgb_jpeg(image: torch.Tensor) -> torch.Tensor:
    r"""Convert an RGB image to YCbCr.
...
class Jpeg_ARWGAN(nn.Module):
    def __init__(self,factor):
        super(Jpeg_ARWGAN, self).__init__()
        self.factor=factor
    ...
    def forward(self, noise_and_cover):
        ...
        n_a_c[0]=torch.FloatTensor(jpeg_image).to(self.device)
        return n_a_c
